...
echarts.init(document.getElementById('area')).setOption({
	...
	series: [{
		...
		center: ['50%', '56%'],
		//label: { show: false },
		label: { rotate: 0, fontSize: 18, formatter: function formatter(res) {
				return (res.name==='东'?'     ':'') + res.name + '区：' + (res.value / res.treePathInfo[0].value * 100).toFixed(2) + '%' + (res.name==='西'?'     ':'');
			} },
...
echarts.init(document.getElementById('post')).setOption({
	...
	series: {
		...
		label: { formatter: '{b}：{d}%', fontSize: 18 },
