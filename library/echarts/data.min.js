...
for (var _i = 0; _i < 32; _i++) {
	...
	data.east.push(data[dataNameE[nowMonth]][_i]); // 因为需要有跳过的需求，所以有一个作为循环，需要加入字符串，也就多一天，在建立xDate的时候加完日期删掉
...
var day = data.all.length - 1; // 所有天数，标题和旭日图算最后一天会用到
for (var _i2 = 1; _i2 <= day; _i2++) {	// 从1开始在日期是字符串的时候有用，会差一位
	...
	if(isNaN(data.east[_i2])) {	// 如果东这一天是个字符串则跳到指定的逗号后面的天数并删掉自己，否则就++
		today = data.east[_i2].replace(/\d*,/, '')
		data.east.splice(_i2, 1)
	} else {
		today = today - 0 + 1
	}
...
excel.split('\n').forEach(function (item, index) {
	...
	if (res[0] && res[1] && res[3] && res[5]) {
		...
		// 职位简洁对外版
		if (/总经理|[^]*董事[^]*|总裁|创始人|CEO/i.test(res[5])) {
			excelPostData2[0].value++;
		} else if (/法人/.test(res[5])) {
			excelPostData2[1].value++;
		} else if (/[^]*副总[^]*|投资人|[^]*股东/.test(res[5])) {
			excelPostData2[2].value++;
		} else if (/[^]*总监|[^]*主管/.test(res[5])) {
			excelPostData2[3].value++;
		} else if (/[^]*经理/.test(res[5])) {
			excelPostData2[4].value++;
		} else {
			excelPostData2[5].value++;
		}
	}
});
...
//excelPostData2.sort(function (a, b) {
//	return b.value - a.value;
//});
// 省市柱形图需要的X轴数据
var excelProName = [];
